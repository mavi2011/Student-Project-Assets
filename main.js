...
function draw(){
    ...
    if(mouseIsPressed)
    {
        if(pmouseX, pmouseY, mouseX, mouseY);
    }
}
...
function gotResult(error, results){ 
    if (error) {
     ...
     console.log(results);
      document.getElementById('your sektch').innerHTML = 'your sektch: ' + results[0].yoursektch;
      document.getElementById('confidence').innerHTML = 'Confidence: ' + Math.round(results[0].confidence * 100) + '%'; 
      utterThis = new SpeechSynthesisUtterance(results[0].yousektch);
